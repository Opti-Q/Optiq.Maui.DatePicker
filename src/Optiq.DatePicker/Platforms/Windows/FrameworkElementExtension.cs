...
namespace Optiq.DatePicker.Platforms
{
	internal static class FrameworkElementExtension
	{
		...
		static DependencyProperty? GetForegroundProperty(FrameworkElement element)
		{
			...
			{
				if (ReflectionExtension.GetFields(type).FirstOrDefault(f => f.Name == "ForegroundProperty") is not FieldInfo field)
		...
		internal static IDisposable OnLoaded(this FrameworkElement frameworkElement, Action action)
		{
			...
			{
				action();
				return new Optiq.DatePicker.Platforms.ActionDisposable(() => { });
			}

			RoutedEventHandler? routedEventHandler = null;
            Optiq.DatePicker.Platforms.ActionDisposable disposable = new Optiq.DatePicker.Platforms.ActionDisposable(() =>
			{
		...
		{
			var rect = new Microsoft.Maui.Graphics.Rect(0, 0, frameworkElement.ActualWidth, frameworkElement.ActualHeight);
		...
		internal static WPoint? GetLocationOnScreen(this IElement element)
		{
			...
			var view = element.ToPlatform(element.Handler.MauiContext);
			return
		...
		internal static WPoint? GetLocationRelativeTo(this IElement element, UIElement relativeTo)
		{
			...
			return
				element
					.ToPlatform(element.Handler.MauiContext)
		...
		internal static WPoint? GetLocationRelativeTo(this IElement element, IElement relativeTo)
		{
			...
			return
				element
					.ToPlatform(element.Handler.MauiContext)
					.GetLocationRelativeTo(relativeTo.ToPlatform(element.Handler.MauiContext));
		}
